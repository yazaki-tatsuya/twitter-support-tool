...
public class TwitterFollowerSearch_V4_AjaxClear extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("# == [SV_④v4] 0-1: Check value before remove : "+session.getAttribute("finalstr"));
		session.removeAttribute("finalstr");
		String searchTarget = request.getParameter("searchUser");
		System.out.println("# == [SV_④v4] 0-2: Check value of search taget : "+searchTarget);
		
		//# エンコーディング方式の設定
		request.setCharacterEncoding("UTF-8");
		response.setHeader("Content-Type", "text/html; charset=UTF-8");
		long[] followerid = fi.getAllFollowersId(searchTarget);
		System.out.println("# == [SV_④v4] 1: Get lenght of follower : "+followerid.length);
