...
<%@ page import="java.util.List,java.util.Date,utils.RoutingTable,models.HashTagSearchList" %>
<!DOCTYPE html>
<% 
...
HashTagSearchList ht = (HashTagSearchList)request.getAttribute("HashTagSearchList");
session.setAttribute("HashTagSearchList", request.getAttribute("HashTagSearchList"));
session.setAttribute("hashtag_val", request.getAttribute("hashtag"));
session.setAttribute("favcount_val", request.getAttribute("favcount"));
System.out.println("# ====== Get query key in result screen : "+request.getAttribute("hashtag")+" & "+request.getAttribute("favcount"));
%>
...
<table border="1" width="250" class="center">
	<tr>
		...
		<td width="150">
			<div class="center"><%=request.getAttribute("hashtag") %><br />
			「<%=request.getAttribute("favcount") %>」いいね以上</div>
		</td>
	</tr>
</table>
<br />
<input type="button" value="csvダウンロード（PC限定）" onclick="location.href='hashtag_search_csv2'" style="float: left" >
...
<table border="1" width="100%" >
	...
	for(int i=0; i<ht.getTweetId().size(); i++){
		String url_user = "https://twitter.com/"+ht.getUser().get(i);
		String url_tweet = "https://twitter.com/"+ht.getUser().get(i)+"/status/"+ht.getTweetId().get(i);
	...
	<tr>
		<td><%=i+1%></td>
		<td><%=ht.getDate().get(i) %></td>
		<td><a href=<%=url_user%> target="_blank">@<%=ht.getUser().get(i)%></a></td>
		<td><a href=<%=url_tweet%> target="_blank">★URL</a></td>
		<td><%=ht.getContent().get(i) %></td>
		<td><%=ht.getFavCnt().get(i) %></td>
		<td><%=ht.getRetCnt().get(i) %></td>
<%-- 
		<td><%=i+1%></td>
